    ...
    public partial class ctrlAddEditPerson : UserControl
    {
        ...
        private void SaveImage()
        {
            ...
            if (openFileDialog1.ShowDialog() == DialogResult.OK)
            {
                ...
                pbPersonImage.Image =  Image.FromFile(SourceFilePath);
                ...
                string extension = Path.GetExtension(SourceFilePath), NewFileName = Guid.NewGuid().ToString() + extension;
                ...
                try
                {
                    File.Copy(SourceFilePath, destinationFilePath, true); // 'true' to overwrite if it exists
                    MessageBox.Show("File copied successfully!");
                }
                catch (Exception ex)
                {
                    MessageBox.Show($"Error copying file: {SourceFilePath}");
                }
            }
        }
