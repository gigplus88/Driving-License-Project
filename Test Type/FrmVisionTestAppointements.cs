...
namespace DVLD.Test_Type
{
    public partial class FrmVisionTestAppointements : Form
    {
        ...
        private void btnNewVisionTest_Click(object sender, EventArgs e)
        {
            ...
            if (clsTestAppointment.IsHasTestAppointment(PersonLDLAppID)) // if equal 0 is fail and It had Appointment
            {
                if (clsTest.LastVisionTest(PersonLDLAppID, (int)enTestType.Vision) == false)
                {
        ...
        {
            if (clsTest.LastVisionTestBTestAppointmentIDy(TestAppointmentID, (int)enTestType.Vision) == false)
            {
                FrmTakeTest frmTakeTest = new FrmTakeTest(TestAppointmentID, ctrlDrivingLicenseApplication1.LDLAppID,
                                                          ctrlDrivingLicenseApplication1.FullName, ctrlDrivingLicenseApplication1.ClassName);
